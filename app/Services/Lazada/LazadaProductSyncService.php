...
class LazadaProductSyncService
{
    ...
    private function resolveMultiSelectOptionValues(LazadaAttributeMapping $mapping, Product $product, ?int $channelId): array
    {
        ...
        // จาก resolveSingleSelectOptionValue()'s caller ที่ส่งถูกต้อง)
        // resolveFormattedAttributeValue() จะ resolve locale_id เป็น null
        // เสมอถ้าไม่ส่ง localeCode มา ทำให้ PIM multiselect attribute ที่เป็น
        // locale-based ไม่มีวัน match แถวที่เก็บไว้จริง (locale_id ไม่ null)
        // — ค่าจะดูเหมือน "ไม่มีค่า" เสมอทั้งที่สินค้ามีข้อมูลจริง
