...
{
    /**
     * `/catalog/business-types/*` (Business Types master) is gated by its
     * own `business_types.*` permissions. Backfill onto every role that
     * already holds the equivalent `categories.*` grant — same move as
     * backfill_points_permissions. Run `php artisan permissions:sync`
     * afterwards to register the resource for the Roles UI / Administrator.
     */
    public function up(): void
    {
        $this->copyGrant('categories', 'list_categories', 'business_types', 'list_business_types');
        $this->copyGrant('categories', 'edit_categories', 'business_types', 'edit_business_types');

        $roleIds = DB::table('role_permissions')->where('resource', 'business_types')->pluck('role_id')->unique();
    ...
    {
        DB::table('role_permissions')->where('resource', 'business_types')->delete();
    }
