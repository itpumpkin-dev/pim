...
{
    /**
     * `/catalog/vendors/*` (Vendors master) is gated by its own `vendors.*`
    ...
    {
        $this->copyGrant('categories', 'list_categories', 'vendors', 'list_vendors');
        $this->copyGrant('categories', 'edit_categories', 'vendors', 'edit_vendors');

        $roleIds = DB::table('role_permissions')->where('resource', 'vendors')->pluck('role_id')->unique();
    ...
    {
        DB::table('role_permissions')->where('resource', 'vendors')->delete();
    }
