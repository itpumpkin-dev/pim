...
{
    /**
     * `/catalog/commission-groups/*` (Commission Groups master) is gated by
     * its own `commission_groups.*` permissions. Backfill onto every role
    ...
    {
        $this->copyGrant('categories', 'list_categories', 'commission_groups', 'list_commission_groups');
        $this->copyGrant('categories', 'edit_categories', 'commission_groups', 'edit_commission_groups');

        $roleIds = DB::table('role_permissions')->where('resource', 'commission_groups')->pluck('role_id')->unique();
    ...
    {
        DB::table('role_permissions')->where('resource', 'commission_groups')->delete();
    }
