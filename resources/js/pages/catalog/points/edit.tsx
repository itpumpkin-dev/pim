...
export default function PointEdit({ point }: Props) {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: t('editPoint'), href: '#' },
    ];

    const { data, setData, put, processing, errors, isDirty } = useForm({
        point_type: point.point_type ?? '',
        point_ratio: String(point.point_ratio ?? ''),
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`${t('editPoint')}: ${point.point_type}`} />
            <Box component="form" onSubmit={submit} sx={{ p: { xs: 2, md: 4 }, width: '100%', maxWidth: 860, bgcolor: FIORI.pageBg }}>
                <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ sm: 'center' }} spacing={2} sx={{ mb: 3 }}>
                    <Typography variant="h5" fontWeight={600} sx={{ color: FIORI.textPrimary }}>{t('editPoint')}</Typography>
