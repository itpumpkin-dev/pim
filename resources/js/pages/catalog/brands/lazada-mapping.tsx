...
import { LazadaBrandPicker, type LazadaBrandOption } from '@/components/catalog/lazada-brand-picker';
...
/**
 * Lazada sibling of shopee-mapping.tsx/woocommerce-mapping.tsx — identical
 * shape, just a different picker/endpoint. Kept as its own file rather than
 * a shared parameterized component, same as the Category mapping pages.
 */
export default function LazadaBrandMapping({ brands, stats, filters }: Props) {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: t('lazadaBrandMappingTitle'), href: '#' },
    ];
    ...
    const [pending, setPending] = useState<Record<number, LazadaBrandOption | null>>({});
    ...
    useEffect(() => {
        ...
        const timeout = setTimeout(() => {
            router.get('/catalog/brands/lazada-mapping', { search, status, per_page: perPage, only_with_products: onlyWithProducts }, { preserveState: true, replace: true });
        }, 300);
    ...
    const applyStatus = (value: 'unmapped' | 'mapped' | 'all') => {
        setStatus(value);
        router.get('/catalog/brands/lazada-mapping', { search, status: value, per_page: perPage, only_with_products: onlyWithProducts }, { preserveState: true });
    };

    const applyOnlyWithProducts = (value: boolean) => {
        setOnlyWithProducts(value);
        router.get('/catalog/brands/lazada-mapping', { search, status, per_page: perPage, only_with_products: value }, { preserveState: true });
    };

    const handlePerPageChange = (value: number) => {
        setPerPage(value);
        router.get('/catalog/brands/lazada-mapping', { search, status, per_page: value, only_with_products: onlyWithProducts }, { preserveState: true });
    };

    const goToPage = (page: number) => {
        router.get('/catalog/brands/lazada-mapping', { search, status, per_page: perPage, page, only_with_products: onlyWithProducts }, { preserveState: true });
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={t('lazadaBrandMappingTitle')} />
            <Box sx={{ p: 4 }}>
                <Stack direction="row" justifyContent="space-between" alignItems="flex-start" sx={{ mb: 3 }}>
                    <Box>
                        ...
                        <Typography variant="h4" fontWeight={700}>{t('lazadaBrandMappingTitle')}</Typography>
