...
interface Props {
    attributeApiSource: AttributeApiSource;
}

export default function AttributeApiSourceEdit({ attributeApiSource }: Props) {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: t('editAttributeApiSourceTitle'), href: '#' },
    ];

    const { data, setData, put, processing, errors, isDirty } = useForm<ApiSourceForm>({
        name: attributeApiSource.name || '',
        endpoint: attributeApiSource.endpoint || '',
        method: attributeApiSource.method || 'GET',
        auth_type: attributeApiSource.auth_type || 'none',
        credentials: {},
        ...
        is_active: Boolean(attributeApiSource.is_active),
    });
    const skipNavigationGuardRef = useUnsavedChangesGuard(isDirty);

    const [testing, setTesting] = useState(false);
    const [testRows, setTestRows] = useState<TestResultRow[] | null>(null);
    ...
    const credentialPlaceholder = (key: string) => (attributeApiSource.credentials_set[key] ? t('apiSourceCredentialSetHint') : '');

    const runTest = () => {
        setTesting(true);
        setTestError(null);
        fetch(`/catalog/attributes/api-sources/${attributeApiSource.id}/test`, { headers: { Accept: 'application/json' } })
            .then(async (res) => {
    ...
    const submit = (event: FormEvent) => {
        ...
        put(`/catalog/attributes/api-sources/${attributeApiSource.id}`, {
            replace: true,
            onFinish: () => {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`${t('editAttributeApiSourceTitle')}: ${attributeApiSource.name}`} />
            <Box component="form" onSubmit={submit} sx={{ p: { xs: 2, md: 4 }, bgcolor: FIORI.pageBg, minHeight: '100%', width: '100%', maxWidth: 760 }}>
                <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ sm: 'center' }} spacing={2} sx={{ mb: 3 }}>
                    <Typography variant="h5" fontWeight={600} sx={{ color: FIORI.textPrimary }}>{t('editAttributeApiSourceTitle')}</Typography>
                    ...
                    <FioriFormGroup title={t('generalTitle')}>
                        ...
                        {data.auth_type === 'api_key_header' && (
                            <>
                                ...
                                <FioriField label={t('apiSourceApiKeyLabel')} htmlFor="api-source-api-key" hint={credentialPlaceholder('api_key')} valueState={valueStateOf(errors['credentials.api_key'])} message={errors['credentials.api_key']}>
                                    <TextField id="api-source-api-key" type="password" fullWidth size="small" placeholder={credentialPlaceholder('api_key')} value={data.credentials.api_key ?? ''} onChange={(e) => setCredential('api_key', e.target.value)} />
                                </FioriField>
                            </>
                        )}

                        {data.auth_type === 'bearer' && (
                            <FioriField label={t('apiSourceTokenLabel')} htmlFor="api-source-token" hint={credentialPlaceholder('token')} valueState={valueStateOf(errors['credentials.token'])} message={errors['credentials.token']}>
                                <TextField id="api-source-token" type="password" fullWidth size="small" placeholder={credentialPlaceholder('token')} value={data.credentials.token ?? ''} onChange={(e) => setCredential('token', e.target.value)} />
                            </FioriField>
                        )}
                        ...
                        {data.auth_type === 'basic' && (
                            <>
                                ...
                                <FioriField label={t('apiSourcePasswordLabel')} htmlFor="api-source-password" hint={credentialPlaceholder('password')} valueState={valueStateOf(errors['credentials.password'])} message={errors['credentials.password']}>
                                    <TextField id="api-source-password" type="password" fullWidth size="small" placeholder={credentialPlaceholder('password')} value={data.credentials.password ?? ''} onChange={(e) => setCredential('password', e.target.value)} />
                                </FioriField>
                            </>
                        )}
                    </FioriFormGroup>
                        ...
                        <FioriField label="">
                            <Stack spacing={1.5}>
                                <Button
                                    ...
                                    sx={{ alignSelf: 'flex-start', ...fioriDefaultSx }}
                                >
                                    {testing ? t('testingConnection') : t('testConnection')}
                                </Button>

                                {testError && <FioriMessageStrip severity="error">{t('testConnectionFailed', { error: testError })}</FioriMessageStrip>}

                                {testRows !== null && !testError && testRows.length === 0 && (
                                    <FioriMessageStrip severity="warning">{t('testConnectionEmpty')}</FioriMessageStrip>
                                )}

                                {testRows !== null && testRows.length > 0 && (
                                    <>
                                        <FioriMessageStrip severity="success">{t('testConnectionSuccess', { count: testRows.length })}</FioriMessageStrip>
                                        <Paper variant="outlined" sx={{ borderColor: FIORI.border }}>
                                            <Table size="small">
                                                <TableHead>
                                                    <TableRow>
                                                        <TableCell>{t('testConnectionPreviewCode')}</TableCell>
                                                        <TableCell>{t('testConnectionPreviewLabel')}</TableCell>
                                                    </TableRow>
                                                </TableHead>
                                                <TableBody>
