...
interface ProductDetailAttributeRow {
    ...
    value: string | null;
    /** Lazada's own raw input_type (text/numeric/richText/singleSelect/...) — null ถ้าไม่รู้ (เช่น ฟิลด์ตายตัวบางตัว) */
    type?: string | null;
}
...
/** ทุกร้าน Lazada ที่มีในระบบ (ไม่ใช่แค่ร้านที่สินค้านี้ publish อยู่แล้ว) — ให้แท็บ "ร้านค้า" ติ๊ก publish/push/deactivate ได้เองในที่เดียว */
...
interface ProductDetailData {
    ...
    lazada_shops: ProductDetailShop[];
}
...
interface Props {
    ...
    /** ทุกร้าน Lazada ในระบบ — ใช้เป็นตัวเลือกใน dialog "Push ที่เลือก" (bulk) ด้านล่างตาราง */
    lazadaShops: { id: number; name: string }[];
}
...
export default function LazadaProductsMapping({ products, stats, filters, lazadaShops }: Props) {
    const { t: tNav } = useTranslation('nav');
    const { t: tGrid } = useTranslation('grid');

    // ไอคอนแก้ไขสินค้าข้างชื่อในตาราง — พาไปหน้า Edit Product ตรงๆ (products/edit.tsx)
    // ไม่ใช่หน้า mapping นี้ — เช็คสิทธิ์แยกจากสิทธิ์ marketplace_lazada ของหน้านี้เอง
    ...
    const checkShopStatus = (productId: number, shopId: number) => {
        ...
        fetch(`/catalog/products/${productId}/lazada-status/${shopId}`, { headers: { Accept: 'application/json' } })
            .then(async (res) => {
                const body = await res.json();
                // เช็ค detailProductIdRef ก่อน setState เสมอ เหมือน fetch handler อื่นๆ
                // ในไฟล์นี้ (openDetail/refreshDetail/toggleShopPublished ด้านบน) — ไม่งั้น
                // ถ้าปิด drawer แล้วเปิดสินค้าอื่นที่มีร้านเดียวกัน (shopId ซ้ำ) ก่อน
                // response เก่าจะกลับมา ผลเช็คสถานะของสินค้าเก่าจะทับ state ของสินค้า
                // ใหม่ที่กำลังเปิด confirm dialog อยู่ (บั๊กที่เจอจาก code review)
    ...
    const confirmShopAction = () => {
        ...
        const productId = detailProductId;
        const routeSegment = action === 'push' ? 'push-lazada' : 'deactivate-lazada';
        setShopActionConfirm(null);
        ...
        fetch(`/catalog/products/${productId}/${routeSegment}/${shopId}`, {
            ...
            .then(async (res) => {
                ...
                if (!res.ok || !body.job_id) {
                    setDetailPushResult({
                        severity: 'error',
                        message: body.message ?? (action === 'push' ? 'ส่งไป Lazada ไม่สำเร็จ' : 'ปิดการขายบน Lazada ไม่สำเร็จ'),
                    });
                    setActingShopId(null);
                    return;
                }
    ...
    // "แนะนำหมวดหมู่จาก Lazada" — เรียก /product/category/suggestion/get จริง
    // (ดู LazadaAttributeMappingController::categorySuggestions()) แค่เสนอ
    // ตัวเลือกให้กดเลือกแทนการเดินหา manual ใน MarketplaceCategoryPicker เอง —
    // ไม่ได้บันทึกอะไรทันทีที่กด แค่ set selectedLazadaCatId (ค่าเดียวกับที่
    // picker เขียน) ผู้ใช้ยังต้องกด "บันทึก Category Mapping" ตามปกติเหมือนเดิม
    ...
    // ไม่แตะ Payload Lazada (ฟิลด์ตายตัว name/price/qty/... — Section ถัดไป)
    ...
    // call ไป Lazada จริงทุกครั้ง — เผื่อ rate limit เหมือนที่ endpoint อื่นของ
    // Lazada เคยเจอมาแล้ว (ดู LazadaProductSyncService::syncLiveStatus()'s
    // docblock) ไม่ควรยิงอัตโนมัติทุกครั้งที่เปิดดูสินค้าตัวหนึ่งๆ
    const loadCategorySuggestions = () => {
        if (!activeProduct) return;
        setLoadingCategorySuggestions(true);
        setCategorySuggestionError(null);
        fetch(`/catalog/marketplace/lazada/products/${activeProduct.id}/category-suggestions`, { headers: { Accept: 'application/json' } })
            .then(async (res) => {
                const body = await res.json();
    ...
    const clearCategoryMapping = async () => {
        ...
        // categories/lazada-mapping ปกติที่ใช้ตอนบันทึก) จะไปหาเองว่าหมวดหมู่
        ...
        // ไม่ใช่ตัวที่ผูก mapping จริงถ้าข้อมูลมาจากที่อื่น (เช่นหน้า mapping
        // เดิม) — ดู CategoryController::clearProductMarketplaceCategory()'s
        // docblock เต็มๆ ว่าทำไมถึงเคยกดล้างแล้ว field ที่ Lazada บังคับยังโชว์
        // อยู่ในหน้า Edit Product เหมือนเดิมทุกอย่าง
        router.post(
            '/catalog/categories/lazada-mapping/clear-for-product',
    ...
    const syncAttributeFamily = async () => {
        ...
        const confirmed = await confirm({
            ...
            message: 'การกดปุ่มนี้อาจสร้าง PIM Attribute ใหม่หลายตัวสำหรับ Lazada attribute ที่ยังไม่มีใครแมป ต้องการดำเนินการต่อหรือไม่?',
            severity: 'warning',
    ...
    // Lazada) ที่กำลังโชว์อยู่ตอนนี้ในครั้งเดียว — ต่างจาก clearAttribute()
    // ด้านบนที่ล้างทีละแถว ส่ง mappings หลายรายการไปพร้อมกันได้ในคำขอเดียว (ดู
    // LazadaAttributeMappingController::update() — validate เป็น array อยู่แล้ว)
    // ไม่แตะ Payload Lazada (ฟิลด์ตายตัว name/price/qty/... จัดการแยกด้วย
    ...
    const clearAllAttributeMappings = async () => {
        const mappedRows = (lazadaAttributes ?? []).filter((a) => a.mapped);
        if (mappedRows.length === 0) return;

        const confirmed = await confirm({
            title: 'ล้าง Attribute Mapping',
            message: `ล้างการแมป PIM attribute ออกจาก Lazada attribute ทั้งหมด ${mappedRows.length} รายการที่แมปไว้ในหมวดหมู่นี้? mapping เป็นระดับ global เลยจะมีผลกับสินค้าทุกตัวที่ใช้ attribute เหล่านี้ ต้องแมปใหม่เองทีละตัว ต้องการดำเนินการต่อหรือไม่?`,
            severity: 'warning',
        ...
        fetch('/catalog/attributes/lazada-mapping', {
            ...
            .then((res) => {
                ...
                setLazadaAttributes((prev) => (prev ? prev.map((a) => (a.mapped ? { ...a, mapped: null } : a)) : prev));
            })
                        ...
                        {canEditProducts && (
                            <Tooltip title="แก้ไขสินค้า">
                                <IconButton
                                    ...
                                    onClick={(e) => {
                                        e.stopPropagation();
                                        router.visit(`/catalog/products/${row.id}/edit?platform=lazada`);
                                    }}
                                >
                                    ...
                                    {canEditProducts && (
                                        <Tooltip title="แก้ไขสินค้า">
                                            <IconButton
                                                size="small"
                                                sx={fioriIconButtonSx}
                                                onClick={() => router.visit(`/catalog/products/${activeProduct.id}/edit?platform=lazada`)}
                                            >
                                                <EditIcon fontSize="inherit" />
                                            </IconButton>
                                        </Tooltip>
                        ...
                        <Stack spacing={3} sx={{ px: { xs: 2, md: 4 }, pt: 3 }}>
                            ...
                            >
                                <Stack spacing={2.5}>
                                    ...
                                    {!activeProduct.master_category && (
                                        <AssignPimCategoryPanel
                                            ...
                                            onAssigned={(category) =>
                                                setActiveProduct((prev) =>
                                                    prev
                                                        ? { ...prev, master_category: { id: category.id, name: category.name, path: category.path, lazada_category_id: null } }
                                                        : prev,
                                                )
                                            }
                                        />
                                    ...
                                    {/* แนะนำหมวดหมู่จาก Lazada จริง (/product/category/suggestion/get)
                                    — เลือกจากรูป+ชื่อสินค้าตัวนี้ กดแล้วแค่ตั้งค่า
                                    selectedLazadaCatId เหมือน picker ด้านบน ยังต้อง
                                    ...
                                    <Box>
                                        <Button
                                            ...
                                            sx={fioriDefaultSx}
                                        >
                                            {loadingCategorySuggestions ? 'กำลังค้นหา...' : 'แนะนำหมวดหมู่จาก Lazada'}
                                        </Button>
                                        ...
                                        {categorySuggestions && (
                                            categorySuggestions.length === 0 ? (
                                                <Typography variant="body2" sx={{ color: FIORI.textSecondary, mt: 1 }}>
                                                    Lazada ไม่มีคำแนะนำให้สำหรับสินค้านี้
                                                </Typography>
                                            ) : (
                                                <Stack direction="row" flexWrap="wrap" gap={1} sx={{ mt: 1 }}>
                                                    {categorySuggestions.map((s) => (
                                                        <Chip
                                                            ...
                                                            onClick={() => setSelectedLazadaCatId(s.category_id)}
                                                            sx={
                                                                selectedLazadaCatId === s.category_id
                                    ...
                                    <Stack spacing={3}>
                                        <Stack direction="row" justifyContent="space-between" alignItems="center">
                                            ...
                                            <Stack direction="row" spacing={1}>
                                                ...
                                                {(lazadaAttributes ?? []).some((a) => a.mapped) && (
                                                    <Button
                                                        size="small"
                                                        variant="outlined"
                                                        color="error"
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
                    ...
                    <Box sx={{ display: 'flex', flexDirection: 'column', height: '100%', bgcolor: FIORI.pageBg }}>
                                        ...
                                        {canEditProducts && (
                                            <Tooltip title="แก้ไขสินค้า">
                                                <IconButton
                                                    size="small"
                                                    sx={fioriIconButtonSx}
                                                    onClick={() => router.visit(`/catalog/products/${detailProductId}/edit?platform=lazada`)}
                                                >
                                                    <EditIcon fontSize="inherit" />
                                                </IconButton>
                                            </Tooltip>
                        ...
                        <Box sx={{ flex: 1, overflowY: 'auto', p: 2 }}>
                            {loadingDetail ? (
                                ...
                                <Stack spacing={1}>
                                    {detailData.lazada_shops.map((shop) => (
                                        <Box
                                            key={shop.id}
                                            sx={{
                                                p: 1.5,
                                    ...
                                    {detailData.lazada_shops.length === 0 && (
                                        <Typography variant="body2" sx={{ color: FIORI.textSecondary }}>
                                            ยังไม่มีร้าน Lazada เชื่อมต่อไว้ในระบบเลย
                                        </Typography>
                                    )}
                                </Stack>
                            ) : (
                        ...
                        >
                            {canEditProducts ? (
                                <Button size="small" onClick={() => router.visit(`/catalog/products/${detailProductId}/edit?platform=lazada`)} sx={fioriGhostSx}>
                                    แก้ไข
                                </Button>
                            ) : (
                                <Box />
            ...
            product ย่อยๆ) เพราะกดครั้งเดียวมีผลจริงกับ listing บน Lazada เลย — ใช้
            ...
            >
                <DialogTitle>
                    {shopActionConfirm?.action === 'push' ? 'ตรวจสอบข้อมูลก่อนส่งไป Lazada' : 'ปิดการขายบน Lazada?'}
                    {detailData && (
                ...
                <DialogContent dividers>
                    {shopActionConfirm?.action === 'deactivate' && (
                        <DialogContentText>
                            การดำเนินการนี้จะปิดการขาย (ไม่ลบ) ประกาศขายบน Lazada สำหรับร้าน &quot;{shopActionConfirm?.shopName}&quot;
                        </DialogContentText>
                    )}
                    ...
                    {shopStatusCheck && shopActionConfirm && shopStatusCheck.shopId === shopActionConfirm.shopId && (
                        <Box sx={{ mb: shopActionConfirm.action === 'push' ? 2 : 0 }}>
                            {shopStatusCheck.loading ? (
                                ...
                                <Alert severity="success">กำลังขายอยู่บน Lazada แล้ว — การดำเนินการนี้จะอัปเดตของเดิม</Alert>
                            ) : (
                                    ...
                                    {missing.length > 0 ? (
                                        <Alert severity="error" sx={{ mb: 2 }}>
                                            <Typography variant="body2" fontWeight={600}>
                                                ยังกรอกข้อมูลที่ Lazada บังคับไม่ครบ {missing.length} รายการ: {missing.map((a) => a.label).join(', ')}
                                            </Typography>
                                            <Typography variant="caption" sx={{ display: 'block', mt: 0.5 }}>
                                                ยังกด &quot;ยืนยันส่ง&quot; ต่อได้ แต่ Lazada อาจปฏิเสธถ้าข้อมูลไม่ครบจริง
                                            </Typography>
                                        </Alert>
                                    ) : (
                                        <Alert severity="success" sx={{ mb: 2 }}>ข้อมูลที่ Lazada บังคับครบถ้วนแล้ว</Alert>
                                    )}
            ...
            <FioriMessageBox
                ...
                title={`Push ${selectedRows.length} รายการที่เลือกไป Lazada?`}
                severity="confirm"
            ...
            >
                เลือกร้าน Lazada ที่จะ push สินค้าทั้ง {selectedRows.length} รายการไปด้วยกัน — สินค้าที่ยังไม่ได้ publish ไว้กับร้านนี้จะถูก publish ให้อัตโนมัติก่อน push
                <Select
                    fullWidth
                    size="small"
                    displayEmpty
