...
export default function CommissionGroupCreate() {
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: t('createCommissionGroup'), href: '/catalog/commission-groups/create' },
    ];

    const { data, setData, post, processing, errors, isDirty } = useForm({
        code: '',
        p_group_name: '',
        divisor_start: '0' as string,
        divisor_secondary: '0' as string,
        start_date: '',
        end_date: '',
    ...
    const submit = (event: FormEvent) => {
        ...
        post('/catalog/commission-groups', {
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={t('createCommissionGroup')} />
            <Box component="form" onSubmit={submit} sx={{ p: { xs: 2, md: 4 }, width: '100%', maxWidth: 620, bgcolor: FIORI.pageBg }}>
                <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ sm: 'center' }} spacing={2} sx={{ mb: 3 }}>
                    <Typography variant="h5" fontWeight={600} sx={{ color: FIORI.textPrimary }}>{t('createCommissionGroup')}</Typography>
