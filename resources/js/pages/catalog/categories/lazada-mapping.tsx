...
interface Props {
    ...
    filters: { status: 'unmapped' | 'mapped' | 'all'; search: string; per_page: number };
}
...
export default function LazadaCategoryMapping({ categories, stats, filters }: Props) {
    ...
    useEffect(() => {
        ...
        const timeout = setTimeout(() => {
            router.get('/catalog/categories/lazada-mapping', { search, status, per_page: perPage }, { preserveState: true, replace: true });
        }, 300);
    ...
    const applyStatus = (value: 'unmapped' | 'mapped' | 'all') => {
        setStatus(value);
        router.get('/catalog/categories/lazada-mapping', { search, status: value, per_page: perPage }, { preserveState: true });
    };

    const handlePerPageChange = (value: number) => {
        setPerPage(value);
        router.get('/catalog/categories/lazada-mapping', { search, status, per_page: value }, { preserveState: true });
    };

    const goToPage = (page: number) => {
        router.get('/catalog/categories/lazada-mapping', { search, status, per_page: perPage, page }, { preserveState: true });
    };
