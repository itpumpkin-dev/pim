...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'ADD PROVIDER', href: '/system/translationProviders/create' },
];

export default function TranslationProviderCreate({ providerTypes }: Props) {
    ...
    const submit = (e: FormEvent) => {
        e.preventDefault();
        post('/system/translationProviders', { replace: true });
    };
    ...
    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title="Add Translation Provider" />
            <Box component="form" onSubmit={submit} sx={{ p: { xs: 2, md: 4 }, bgcolor: 'background.default', minHeight: '100%' }}>
                ...
                <Stack spacing={3} sx={{ maxWidth: 800 }}>
                    ...
                    <Paper variant="outlined" sx={{ p: 3, borderRadius: 2, bgcolor: '#fff' }}>
                        ...
                        <Stack spacing={2}>
                            ...
                            <TextField
                                ...
                                fullWidth
                                size="small"
                                placeholder="e.g. Google Cloud Translate (production)"
                                value={data.name}
                    ...
                    {/* Credentials Panel — fields are entirely driven by the selected provider type's schema */}
                    {fields.length > 0 && (
                        <Paper variant="outlined" sx={{ p: 3, borderRadius: 2, bgcolor: '#fff' }}>
                            <Typography variant="h6" fontWeight={700} color="text.primary" sx={{ mb: 2 }}>
                                Credentials
                                            ...
                                            <Tooltip title="Load options from server">
                                                <span>
                                                    <IconButton
                                                        ...
                                                        sx={{ mt: 0.5 }}
                                                    >
                                                        {loadingField === field.key ? <CircularProgress size={20} /> : <RefreshIcon fontSize="small" />}
                                                    </IconButton>
                                                </span>
                                            </Tooltip>
                                        </Stack>
